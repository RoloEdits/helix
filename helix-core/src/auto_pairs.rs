...
pub fn hook(
    ...
    if let Some(pair) = pairs.get(ch) {
        if pair.same() {
            ...
            // && char_at pos == close
            return Some(handle_close(doc, selection, pair, auto_closing_overtype));
        }
    }
...
fn handle_close(
    ...
    let transaction = Transaction::change_by_selection(doc, selection, |start_range| {
        ...
        let change = if next_char == Some(pair.close) && auto_closing_overtype {
